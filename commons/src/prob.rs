...
pub fn select_weighted_by<'a, R: rand::Rng, K>(
    rng: &mut R,
    candidates: &'a Vec<K>,
    f: fn(&'a K) -> f32,
) -> Option<&'a K> {
    let weights: Vec<f32> = candidates.iter().map(|i| f(i)).collect();
    select_one(rng, &weights).map(|index| &candidates[index])
}
...
mod test {
    ...
    #[test]
    fn select_one_should_return_one_of_candidates_inverse_list() {
        let mut rng = thread_rng();
        let list = vec![1.0, 10.0];
